...
def compute_building_overview(address: str, scale_mw: float, center_type: str) -> Dict[str, object]:
    ...
    building_area = round(site_area * coverage_rate / 100)
...
def compute_mep_overview(overview: dict) -> pd.DataFrame:
    total_building_area = overview["예상건축면적 (㎡)"]
...
def main():
    ...
    with right:
        ...
        with tabs[0]:
            ...
            st.table(pd.DataFrame([overview]).T.rename(columns={0: "값"}))
